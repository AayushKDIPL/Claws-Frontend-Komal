...
const ProfilePage = () => {
  ...
  const fetchCartData = async (_id, token) => {
    try {
      const response = await fetch(`http://localhost:8000/api/order/${_id}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`,
        ...
        <div className="row">
          ...
          <div className="col-md-6 profile-col">
            <div className="profile-card">
              <img src={`http://localhost:8000/${profileData.images}`} alt="Profile" className="profile-image" />
                    ...
                    <div key={index} className="order-card">
                      <Link to={`/product/${e._id}`} style={{textDecoration: 'none'}}>
                          <div className="order-info">
                            <img src={`http://localhost:8000/${e.images}`} alt={order.productName} className="order-product-image" />
